...
<template>
  <ElForm ref="formRef" :model="model" :rules="rules" size="large" :show-label="false" @keyup.enter="handleSubmit">
    ...
    <ElFormItem prop="confirmPassword">
      <ElInput v-model="model.confirmPassword" type="password" show-password-on="click" placeholder="请再次输入密码" />
    </ElFormItem>
    <ElSpace direction="vertical" :size="18" fill class="w-full">
      <ElButton type="primary" size="large" round block @click="handleSubmit">确认</ElButton>
      <ElButton size="large" round @click="toggleLoginModule('pwd-login')">返回</ElButton>
    </ElSpace>
  </ElForm>
</template>
